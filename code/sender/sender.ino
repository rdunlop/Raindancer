...
void timerCallback() {
  if (enableSender) {
    if (sigcode[step] == 1) {
      PORTB &= ~(1 << PB1);  //digitalWrite(pinIN1, LOW);
#if USE_DOUBLE_AMPLTIUDE
      PORTD |= (1 << PD2);   //digitalWrite(pinIN2, HIGH);
#endif
      PORTD |= (1 << PD5);   //digitalWrite(pinEnable, HIGH);
    } else if (sigcode[step] == -1) {
      PORTB  |= (1 << PB1);  //digitalWrite(pinIN1, HIGH);
      PORTD &= ~(1 << PD2);  //digitalWrite(pinIN2, LOW);
      PORTD |= (1 << PD5);   //digitalWrite(pinEnable, HIGH);
    } else {
      PORTD &= ~(1 << PD5);  //digitalWrite(pinEnable, LOW);
    }
    PORTB |= (1 << PB4);     //digitalWrite(pinLEDGreen1, HIGH); Informs user that interrupt sends signal. 
    step ++;
